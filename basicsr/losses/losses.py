...
from .loss_util import weighted_loss, cal_weight_map_with_ema_result, cal_weight_map_wout_ema_result
...
@LOSS_REGISTRY.register()
class FrameVariationLoss(nn.Module):
    """Frame variation loss.

    Args:
        loss_weight (float): Loss weight for frame variation loss. Default: 1.0.
        reduction (str): Specifies the reduction to apply to the output.
            Supported choices are 'none' | 'mean' | 'sum'. Default: 'mean'.
    """

    def __init__(self, loss_weight=1.0, reduction='mean', order=1, with_wmap=False, ksize=7):
        super(FrameVariationLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
        ...
        self.order = order
        self.ksize = ksize
        self.with_wmap = with_wmap

    def forward(self, pred, target, weight=None, **kwargs):
        """
        Args:
            pred (Tensor): of shape (N, T, C, H, W). Predicted tensor.
            target (Tensor): of shape (N, T, C, H, W). Ground truth tensor.
            weight (Tensor, optional): of shape (N, T, C, H, W). Element-wise weights. Default: None.
        """
        b, n, c, h, w = target.size()
        # (N, T-1, C, H, W)
        diff_pred = (pred[:, 1:, :, :, :] - pred[:, :-1, :, :, :]) ** self.order
        # (N, T-1, C, H, W)
        diff_target = (target[:, 1:, :, :, :] - target[:, :-1, :, :, :]) ** self.order
        if self.with_wmap:
            weight_map = cal_weight_map_wout_ema_result(diff_target.view(-1, c, h, w),
                                                        diff_pred.view(-1, c, h, w),
                                                        ksize=self.ksize).view(b, n - 1, 1, h, w)
            diff_pred = weight_map * diff_pred
            diff_target = weight_map * diff_target
        return self.loss_weight * charbonnier_loss(diff_pred, diff_target, weight, reduction=self.reduction)


@LOSS_REGISTRY.register()
class LocalDiscriminativeLoss(nn.Module):
    """Frame variation loss.
    ...
    def __init__(self, loss_weight=1.0, reduction='mean', ksize=7):
        super(LocalDiscriminativeLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
    ...
    def forward(self, pred, target, pred_ema=None, weight=None, **kwargs):
        """
        Args:
            pred (Tensor): of shape (N, T, C, H, W). Predicted tensor.
            target (Tensor): of shape (N, T, C, H, W). Ground truth tensor.
            pred_ema (Tensor): of shape (N, T, C, H, W). Predicted ema tensor.
            weight (Tensor, optional): of shape (N, T, C, H, W). Element-wise weights. Default: None.
        """
        b, n, c, h, w = target.size()

        if pred_ema is not None:
            weight_map = cal_weight_map_with_ema_result(target.view(-1, c, h, w),
                                                        pred.view(-1, c, h, w),
                                                        pred_ema.view(-1, c, h, w),
                                                        ksize=self.ksize).view(b, n, 1, h, w)
        else:
            weight_map = cal_weight_map_wout_ema_result(target.view(-1, c, h, w),
                                                        pred.view(-1, c, h, w),
                                                        ksize=self.ksize).view(b, n, 1, h, w)
        return self.loss_weight * \
               charbonnier_loss(weight_map * pred, weight_map * target, weight, reduction=self.reduction)
