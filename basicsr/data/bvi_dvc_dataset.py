...
class BVIDVCDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(BVIDVCDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        # each clip has 64 frames starting from 0 to 63
        while (start_frame_idx < 0) or (end_frame_idx > 63):
            center_frame_idx = random.randint(0, 63)
            start_frame_idx = (center_frame_idx - self.num_half_frames * interval)
            end_frame_idx = center_frame_idx + self.num_half_frames * interval
...
@DATASET_REGISTRY.register()
class BVIDVCRecurrentDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(BVIDVCRecurrentDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
        self.num_frame = opt['num_frame']
    ...
    def __getitem__(self, index):
        ...
        if start_frame_idx > 64 - self.num_frame * interval:
            start_frame_idx = random.randint(0, 64 - self.num_frame * interval)
