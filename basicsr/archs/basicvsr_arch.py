...
from basicsr.ops.softsplat import FunctionSoftsplat, ModuleSoftsplat
...
class ConvNextResBlocks(nn.Module):
    """Conv and residual block used in BasicVSR.
    ...
    def __init__(self, num_in_ch=3, num_out_ch=64, num_block=15, act_type='relu'):
        super().__init__()
        self.main = nn.Sequential(
            nn.Conv2d(num_in_ch, num_out_ch, 3, 1, 1, bias=True), nn.LeakyReLU(negative_slope=0.1, inplace=True),
            make_layer(ConvNextResBlock, num_block, inp_channels=num_out_ch, out_channels=num_out_ch, act_type=act_type)
        )
...
@ARCH_REGISTRY.register()
class BasicVSR_DirectUp(nn.Module):
    """A recurrent network for video SR. Now only x4 is supported.
    ...
    def __init__(self, num_feat=64, num_block=15, spynet_path=None):
        ...
        # reconstruction
        self.fusion = nn.Conv2d(num_feat * 2, num_feat, 1, 1, 0, bias=True)
        self.conv_last = nn.Conv2d(num_feat, 4 * 4 * 3, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        for i in range(0, n):
            ...
            out = torch.cat([out_l[i], feat_prop], dim=1)
            out = self.lrelu(self.fusion(out))
            out = self.pixel_shuffle(self.conv_last(out))
            base = F.interpolate(x_i, scale_factor=4, mode='bilinear', align_corners=False)
            out += base
            out_l[i] = out
...
@ARCH_REGISTRY.register()
class IconVSR_DirectUp(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper.
    ...
    def forward(self, x):
        ...
        for i in range(0, n):
            ...
            # upsample
            out = self.pixel_shuffle(self.conv_last(feat_prop))
            base = F.interpolate(x_i, scale_factor=4, mode='bilinear', align_corners=False)
            out += base
            out_l[i] = out
...
@ARCH_REGISTRY.register()
class BasicVSR_DirectUp_NoFlow(nn.Module):
    """A recurrent network for video SR. Now only x4 is supported.
...
@ARCH_REGISTRY.register()
class IconVSR_DirectUp_NoFlow(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper.
...
@ARCH_REGISTRY.register()
class BasicVSRCouplePropV1(nn.Module):
    """BasicVSRCoupleProp.
...
@ARCH_REGISTRY.register()
class BasicVSRCouplePropV2(nn.Module):
...
@ARCH_REGISTRY.register()
class BasicVSRCouplePropV3(nn.Module):
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCoupleProp_NoFlow(nn.Module):
    """BasicVSRCoupleProp V2.
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV1(nn.Module):
    """BasicVSRCoupleProp.
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV2(nn.Module):
    """BasicVSRCoupleProp V2.
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV3(nn.Module):
    """BasicVSRCoupleProp V3.
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV4(nn.Module):
    """BasicVSRCoupleProp V4.
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV5(nn.Module):

    """BasicVSRCoupleProp V5.
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV2_FF(nn.Module):
    """BasicVSRCoupleProp V2.
    ...
    def forward(self, x):
        ...
        for i in range(n - 1, -1, -1):
            ...
            if i < n - 1:
                ...
                feat_prop = FunctionSoftsplat(feat_prop, flow, None, 'average')
            feat_prop = torch.cat([feat_i, feat_prop], dim=1)
            feat_prop = self.backward_trunk(feat_prop)
            out_l.insert(0, feat_prop)
...
@ARCH_REGISTRY.register()
class RealTimeBasicVSRCouplePropV3_FF(nn.Module):
    """BasicVSRCoupleProp V3.
