...
import einops
from basicsr.archs.arch_util import to_2tuple, trunc_normal_, DropPath
...
if __name__ == '__main__':
    # dim, heads, window_size, attn_drop, proj_drop = 32, 1, 8, 0.0, 0.0
    # loc_att = LocalAttention(dim, heads, window_size, attn_drop, proj_drop)
    # inp = torch.randn(1, 32, 64, 64)
    # out, _, _ = loc_att(inp)
    # print(out.shape)
    #
    # dim, heads, window_size, attn_drop, proj_drop, shift_size, fmap_size = 32, 1, 8, 0.0, 0.0, 8 // 2, (64, 64)
    # win_att = ShiftWindowAttention(dim, heads, window_size, attn_drop, proj_drop, shift_size, fmap_size)
    # inp = torch.randn(1, 32, 64, 64)
    # out, _, _ = win_att(inp)
    # print(out.shape)

    q_size, kv_size, n_heads, n_head_channels, n_groups = (48, 48), (24, 24), 1, 32 // 1, 1
    attn_drop, proj_drop, stride = 0.0, 0.0, 2
    offset_range_factor, use_pe, dwc_pe = 1, False, False
    no_off, fixed_pe = 0, 0
    dfm_att = DAttentionBaseline(q_size, kv_size, n_heads, n_head_channels, n_groups,
                                 attn_drop, proj_drop, stride, offset_range_factor,
                                 use_pe, dwc_pe, no_off, fixed_pe)
    inp = torch.randn(1, 32, 48, 48)
    out, p, r = dfm_att(inp)
    print(out.shape)

    # fmap_size, window_size, ns_per_pt = (64, 64), 8, None
    # dim_in, dim_embed, depths, stage_spec, n_groups = 32, 32, 1, 'L', 1
    # use_pe, sr_ratio = True, None
    # heads, stride, offset_range_factor = 1, 1, 1
    # dwc_pe, no_off, fixed_pe = False, 0, 0
    # attn_drop, proj_drop, expansion, drop, drop_path_rate, use_dwc_mlp = False, False, 1, 0.0, [0.0], True
    # transformer = TransformerStage(fmap_size, window_size, ns_per_pt,
    #                                dim_in, dim_embed, depths, stage_spec, n_groups,
    #                                use_pe, sr_ratio,
    #                                heads, stride, offset_range_factor,
    #                                dwc_pe, no_off, fixed_pe,
