...
#     def __init__(self, img_size=224, in_chans=3, num_classes=1000, token_dim=32, embed_dim=96, mlp_ratio=4.,
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.ReLU6, stride=False):
        super().__init__()
        self.stride = stride
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
...
class Att(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=1, shuffle=False, qkv_bias=False, qk_scale=None):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // self.num_heads
        self.ws = window_size
...
class Block(nn.Module):
    ...
    def __init__(self, dim, out_dim, num_heads, window_size=1, shuffle=False, mlp_ratio=4., qkv_bias=False,
                 qk_scale=None, act_layer=nn.ReLU6, norm_layer=LayerNorm, stride=False):
        super().__init__()
        mlp_hidden_dim = int(dim * mlp_ratio)

        self.norm1 = norm_layer(dim)
        self.att = Att(dim, num_heads=num_heads, window_size=window_size,
                        shuffle=shuffle, qkv_bias=qkv_bias, qk_scale=qk_scale)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                       out_features=out_dim, act_layer=act_layer, stride=stride)
        self.norm3 = norm_layer(dim)
...
if __name__ == '__main__':
    ...
    # net = ShuffleTransformer().to(device)
    # inp = torch.rand(1, 3, 224, 224).to(device)
    # out = net(inp)
    # print(out.shape)

    device = torch.device('cuda')
    blk1 = Block(64, 64, 8, window_size=8, shuffle=False, mlp_ratio=1).to(device)
    blk2 = Block(64, 64, 8, window_size=8, shuffle=True, mlp_ratio=1).to(device)
    inp = torch.rand(1, 64, 128, 128).to(device)
    out = blk1(inp)
    out = blk2(out)
    print(out.shape)
