...
if __name__ == '__main__':
    ...
    # inp = torch.rand(1, 32, 128, 128).to(device)
    # flo = torch.rand(1, 2, 128, 128).to(device)
    # out = FunctionSoftsplat(inp, flo, None, 'average')
    # print(out.shape)
    ...
    with torch.no_grad():
        for rep in range(repetitions):
            ...
            flow = torch.rand(b, 2, h, w).to(device)
            starter.record()
            # out = FunctionSoftsplat(dummy_input, flow, None, 'average')
            out = flow_warp(dummy_input, flow.permute(0, 2, 3, 1))
            ender.record()
            # WAIT FOR GPU SYNC
