...
class VideoRecurrentGANModel(VideoRecurrentModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('frame_variation_opt'):
            self.cri_var = build_loss(train_opt['frame_variation_opt']).to(self.device)
        else:
            self.cri_var = None

        if train_opt.get('local_discriminative_opt'):
            self.cri_ldl = build_loss(train_opt['local_discriminative_opt']).to(self.device)
        else:
            self.cri_ldl = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            if self.cri_var:
                l_g_var = self.cri_var(self.output, self.gt)
                l_g_total += l_g_var
            ...
            if self.cri_ldl:
                ...
                l_g_ldl = self.cri_ldl(self.output, self.gt, pred_ema=self.output_ema)
                l_g_total += l_g_ldl
...
@MODEL_REGISTRY.register()
class VideoRecurrentSTGANModel(VideoRecurrentModel):
