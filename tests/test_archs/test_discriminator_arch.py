...
from basicsr.archs.discriminator_arch import SpatioTemporalDiscriminator
...
def test_spatialtemporaldiscriminator():
    seq = torch.randn(1, 10, 3, 128, 128)
    net = SpatioTemporalDiscriminator(num_in_ch=3, num_feat=64, num_frame=3, pad_seq=False)
    out = net(seq)
    print(out.shape)
...
if __name__ == '__main__':
    ...
    # test_spatialtemporaldiscriminator()

    seq = torch.randn(1, 10, 3, 128, 128)
    net = SpatioTemporalDiscriminator(num_in_ch=3, num_feat=64, num_frame=3, pad_seq=True)
    out = net(seq)
    print(out.shape)
